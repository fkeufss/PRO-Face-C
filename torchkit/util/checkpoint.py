...
import logging
import torch
from torchkit.head.distfc.partial_fc import PartialFC
...
class CkptLoader(object):
    ...
    @staticmethod
    def load_fusion(fusion, fusion_resume, local_rank):
        """ load pretrain fusion checkpoint
        """
        if not path.isfile(fusion_resume):
            logging.info("Fusion checkpoint %s not exists" % fusion_resume)
        else:
            device = torch.device("cuda:%d" % local_rank)
            fusion.load_state_dict(torch.load(fusion_resume, map_location=device))
            logging.info("Loading Fusion checkpoint %s succeed" % fusion_resume)
...
class CkptSaver(object):
    ...
    def save_backbone(backbone, model_root, epoch, rank):
        ...
        if rank == 0:
            ...
            torch.save(backbone.state_dict(), backbone_path) # .module
            # backbone_path_two = path.join(model_root, "Backbone_all_Epoch_%d_checkpoint.pth" % epoch)
            # torch.save(backbone, backbone_path_two)
    ...
    @staticmethod
    def save_fusion(fusion_model, model_root, epoch, rank):
        if rank == 0:
            model_path = path.join(model_root, "Fusion_Epoch_%d_checkpoint.pth" % epoch)
            torch.save(fusion_model.state_dict(), model_path)

    @staticmethod
    def save_noiseModel(noise_model, model_root, epoch, rank):
        if rank == 0:
            model_path = path.join(model_root, "Noise_Epoch_%d_checkpoint.pth" % epoch)
            torch.save(noise_model.state_dict(), model_path)
