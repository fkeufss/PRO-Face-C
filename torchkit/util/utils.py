...
def all_gather_tensor(input_tensor, dim=0):
    ...
    tensor_size = torch.tensor([input_tensor.shape[0]], dtype=torch.int64).cuda("cuda:1")
    ...
    padded = torch.empty(max_size.item(), *input_tensor.shape[1:], dtype=input_tensor.dtype).cuda("cuda:1")
...
def init_log(output_dir):
    output_name = time.strftime("log_%Y-%m-%d_%H:%M.log", time.localtime())
